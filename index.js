...
// INSTANTSI OPENAI
const openai = new OpenAI({ apiKey: "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" });
...
async function gptChat(prompt) {
    try {
        const response = await openai.chat.completions.create({
            model: "gpt-4.1-mini", // rekomendasi cepat & murah
            messages: [
                { role: "system", content: "You are GPT-5, an AI helper in Telegram bot." },
...
bot.hears(/^gpt/i, async (ctx) => {
    ...
    await ctx.reply("⏳ Tunggu sebentar, sedang berpikir...");

    const result = await gptChat(q);
